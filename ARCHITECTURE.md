...
mindmap
  root(("Outside<br/>backend/src"))
    ...
    ("🗄️ CLAUDE.md")
    ("frontend/")
      ("booking + admin dashboard wired via fetch()")
```
...
### `admin.controller.ts` ✅
**Purpose:** the admin dashboard's entire backend surface — list all appointments (optionally filtered), get one by id, confirm/cancel/complete/mark-no-show/delete, and list all patients (optionally filtered) plus a single patient's profile.
**How it works:** `listAppointments`/`listPatients` read validated/coerced `req.query.{status,search,date_range}`/`req.query.search` and call `listAppointmentsService`/`listPatientsService`; the id-based handlers (`getAppointmentById`, `confirmAppointment`, `cancelAppointment`, `completeAppointment`, `noShowAppointment`, `deleteAppointment`, `getPatientProfile`) read validated/coerced `req.params.id` (a `number`, though Express's own types still say `string` — hence the `as unknown as { id: number }` cast, same reasoning as `req.query` casts elsewhere) and call the matching service function. All of them just format the result as JSON or forward errors to `next(error)`.
...
### `appointment.service.ts` ✅
**Purpose:** owns every piece of business logic touching the `appointments` table — the transactional booking flow, available-slot computation, and the admin CRUD surface: `createAppointmentService`, `getAvailableSlotsService`, `listAppointmentsService`, `getAppointmentByIdService`, `listAppointmentsByPatientIdService`, `confirmAppointmentService`, `cancelAppointmentService`, `completeAppointmentService`, `noShowAppointmentService`, `deleteAppointmentService`.
**How it works:** `createAppointmentService`, inside `withTransaction`, does an atomic `INSERT ... ON CONFLICT (phone) DO NOTHING RETURNING id` against `patients` (falling back to a `SELECT` by phone only on conflict, so concurrent signups for a brand-new phone number can't create duplicate patients), then inserts the appointment. Outside the transaction, it catches a Postgres `DatabaseError` on the `unique_active_appointment` constraint and rethrows it as an `AppError(409)` with a friendly message; `confirmAppointmentService` does the same catch, since re-confirming an old resolved appointment can collide with a different appointment that's since taken its slot. `getAvailableSlotsService` runs a plain (non-transactional) `SELECT appointment_time FROM appointments WHERE appointment_date = $1 AND status IN ('Pending', 'Confirmed')`, then hands the booked times to the pure `utils/generateSlots.ts::generateAvailableSlots`. Every admin read path opens with a private `markPastAppointmentsAsNoShow()` — a lazy sweep-on-read `UPDATE` that flips any `Pending`/`Confirmed` appointment whose date+time has already passed to `NoShow` (there's no cron/scheduler in the project; this runs on-demand instead). `listAppointmentsService` builds a dynamic `WHERE` clause only for filters actually supplied (`status` exact match, `search` as an `ILIKE` across the joined patient's first/last name and phone, `dateRange` as independent `>=`/`<=` bounds) and always `ORDER BY appointment_date, appointment_time`; the status-mutating functions all throw `AppError('Appointment not found', 404)` when the target row doesn't exist, via a shared internal `updateAppointmentStatusService(id, status)`. `completeAppointmentService`/`noShowAppointmentService` additionally gate on `clinicTime.ts::hasAppointmentDateTimePassed()` — 409 if called before the appointment's own scheduled time.
...
### `patient.service.ts` ✅
**Purpose:** owns queries over the `patients` table on its own (not joined with `appointments`) — `listPatientsService` backing the admin dashboard's patients table, `getPatientByIdService` backing the patient profile page.
**How it works:** a plain (non-transactional) `SELECT` directly against `patients`, with the same dynamic-`WHERE`/optional-`search` pattern as `appointment.service.ts::listAppointmentsService` (`ILIKE` across first/last name, phone, and MRN), ordered by `medical_record_number` (registration order).
**If missing:** `admin.controller.ts::listPatients`/`getPatientProfile` have nothing to call — the admin dashboard would have no way to see the patient roster or an individual patient's profile independent of their appointments.
...
### `frontend/` 🗄️
**Purpose:** the marketing site plus two parts that are genuinely wired to the backend via plain `fetch()`: the public booking form (`page/bookappointment.html`) and the whole admin area (`page/admin/` — login, appointments dashboard, patients list, patient profile). The rest of the marketing pages remain static/unwired. See `CLAUDE.md`'s Frontend section for the full breakdown of what each admin page does.
**If missing:** the backend would have zero real caller — booking, the admin dashboard, and patient management would all only be reachable via raw HTTP requests (curl/Postman), not through any UI.
